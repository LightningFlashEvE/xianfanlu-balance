...
本文说明评估器、沙盘、平衡报告如何协作，以及调参时应改哪些文件。系统口径见 [仙凡录系统设定](./GAME_SETTINGS.md)。

## 目标区间（规则预检）

| 场景 | 指标 | 目标胜率 |
|------|------|----------|
| 破境守门 | `realmSweep.vsNextGate`，标准 milestone 养成 vs **独立守门模板** | 38%–72%（爽感约 42–58%） |
...
| `equipment.library.json` | 装备 `quality`、掉落/锻造/升品/词条元数据、`combat` / `growth` |
| `manuals.library.json` | 功法条目、`rank`、`maxRealm`；功法不设物品品阶 |
| `meta.json` | `proficiencyEffectWeight`、标准养成功法累计上限等（**不**经 seed，core 直接读） |
| `sandbox-enemy-presets.ts` | 沙盘模板 + **破境守门** `breakthroughGateTemplateScale` + **章节 BOSS** `chapterBossTemplateScale` |

沙盘默认敌方倍率：`enemyTemplateScale = 0.88`（`schema.prisma` / seed）；破境守门不使用该值，改用 `breakthroughGateTemplateScale`。
...
6. MiniMax 每次响应后累计 `usage`，写入 `BalanceReportCache.tokenUsage`；报告页轮询显示实时 token 总数。
7. 结果写入 `BalanceReportCache` 单例（仅保留最新一份）；可离开页面后返回刷新。
...
| ID | 含义 |
|----|------|
| `milestone_standard` | 当前沙盘境界 + 标准 milestone 满装/上限功法 |
...
报告页显示累计 token 消耗：请求发出时先按 payload 文本写入输入 token 估算，避免长请求期间显示 0；MiniMax 响应后若返回 `usage`，则用真实输入/输出/思考 token 校准本次记录。若接口未返回 usage，则保留估算并记录未返回次数。
